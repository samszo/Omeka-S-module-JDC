...
class JDCViewHelper extends AbstractHelper
{
    ...
    public $querySemanticPositionTarget;
    public $deb;
    public $fin;
    public $rs;
    ...
    public function generer($params)
    {
    ...
    public function createSkosRapports($params)
    {
        //récupère le concept
        $concept = $this->api->read('items', $params['id'])->getContent();

        //récupère l'actant
        if (isset($params['actant'])) {
            $actant = $this->api->read('items', $params['actant']['o:id'])->getContent();
        } else {
    ...
    public function getReseauConceptAll()
    {
    ...
    public function getReseauConcept($params)
    {
        //récupère l'item
        ...
        $this->querySemanticPositionSource = [];
     ...
     * @return int
     */
    public function getNode($item, $niv)
    {
    ...
    public function ajoutRelation()
    {
        ...
        foreach ($this->rs['nodes'] as $n) {
            ...
            if (count($rela) == 0) {
                $this->ajoutLien($this->deb, $n, ['term' => 'deb']);
            }
            ...
            if (count($rela) == 0) {
                $this->ajoutLien($n, $this->fin, ['term' => 'fin']);
            }
        }
    }
     ...
     * @return array
     */
    public function ajoutReseau($item, $niv, $itemBase = false, $target = false)
    {
        //création du noeud de base
        if ($itemBase) {
            $s = $itemBase;
        } else {
            $s = $this->ajoutNoeud(["name" => $item->displayTitle(),"uri" => $item->adminUrl(),"item" => $this->setPropForIHM($item),"type" => $item->resourceClass()->label(),"liens" => []]);
        }
                ...
                foreach ($item->value($r['term'], ['all' => true]) as $cpt) {
                    ...
                    switch ($cpt->type()) {
                        case "resource":
                            ...
                            break;
                        case "uri":
                            $arrN = ["name" => $cpt->asHtml(),"uri" => $cpt->uri(),"type" => $r['term']];
                            //ajoute le noeud
                            $n = $this->ajoutNoeud($arrN);
                            break;
                        default:
                            $arrN = ["name" => $cpt->asHtml(),"type" => $r['term']];
                            //ajoute le noeud
                            $n = $this->ajoutNoeud($arrN);
                            break;
                    }
     ...
     * @return int
     */
    public function ajoutNoeud($arr)
    {
    ...
    public function createDimTxtAsso($params)
    {
    ...
    public function listeDim($params)
    {
    ...
    public function isInExi($listItem, $item)
    {
        foreach ($listItem as $i) {
            if ($i->valueResource()->id() == $item->id()) {
                return 1;
            }
        }
        return 0;
    ...
    public function appendDim($params)
    {
        $children = false;
        $nodes = false;
        switch ($params['dim']) {
    ...
    public function getRapportNodes($oDim)
    {
    ...
    public function addDimToExi($params)
    {
        $oP = $this->getProp('jdc:has' . $params['dim']);
        $newValue[$oP->term()][] = ['value_resource_id' => $params['idDim'],'property_id' => $oP->id(),'type' => 'resource'];
        $this->api->update('items', ($params['idItem'] ? $params['idItem'] : $params['idExi']), $newValue, [], ['isPartial' => true,'collectionAction' => 'append']);
        return $this->api->read('items', $params['idDim'])->getContent();
    }

    public function updateDim($params, $r)
    {
        $newValue = [];
        foreach ($params['data'] as $k => $d) {
            ...
            if ($isNew) {
                $newValue = $this->setValeur($d, $this->getProp($k), $newValue);
            }
            if (!count($rs)) {
                $newValue = $this->setValeur($d, $this->getProp($k), $newValue);
            }
        }
        $rslt = $this->api->update('items', $r->id(), $newValue, [], ['isPartial' => true,'collectionAction' => 'append']);
    ...
    public function childrenDim($r, $propAs, $getChild = true)
    {
        $children = [];
        ...
        foreach ($itemsAsDim as $iDim) {
            ...
            if ($propAs == 'jdc:hasRapport') {
                $vDim['nodes'] = $this->getRapportNodes($rDim);
            }
            $children[] = $vDim;
        }
        return $children;
    ...
    public function linkedDim($r, $prop, $getChild = true)
    {
    ...
    public function findDims($val, $prop, $dim)
    {
        $rs = [];
        $p = $this->getProp($prop);
    ...
    public function setPropForIHM($r)
    {
        $v = $r->getJsonLd();
        $v['id'] = $r->id();
    ...
    public function removeDim($params)
    {
        $oP = $this->getProp('jdc:has' . $params['dim']);
        //récupère les dimensions
        $oDim = $this->api->read('items', $params['idExi'])->getContent();
        ...
        $newValue[$oP->term()] = [];
        foreach ($dims as $d) {
            if ($params['idDim'] != $d['id']) {
                $newValue[$oP->term()][] = ['value_resource_id' => $d['id'],'property_id' => $oP->id(),'type' => 'resource'];
            }
        }
        ...
        foreach ($rapports as $r) {
            ...
            foreach ($r['nodes'] as $n) {
                if ($n['id'] == $params['idDim']) {
                    $delR = true;
                }
            }
            if (!$delR) {
                $newValue[$oPR->term()][] = ['value_resource_id' => $r['id'],'property_id' => $oPR->id(),'type' => 'resource'];
            } else {
                $rs[] = $r;
            }
        }
        //met à jour l'existence
        $this->api->update('items', $params['idExi'], $newValue, [], ['isPartial' => true,'collectionAction' => 'replace']);
        //renvoie la liste des rapports supprimés
        return $rs;
    }

    public function createDim($params, $justeDim = false)
    {
        $oItem = [];
        ...
        if ($justeDim || isset($params['justeDim'])) {
            return $this->setPropForIHM($oDim);
        }
